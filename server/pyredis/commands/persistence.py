...
@command("SAVE", min_args=0, max_args=0, role=Role.ADMIN, complexity="O(N)", is_mutation=False, description="Synchronously save the dataset to disk")
def save_cmd(args: List[Union[bytes, str]], context: CommandContext) -> SimpleString:
    if not context.snapshot:
        raise CommandError("ERR persistence engine not configured")
    context.snapshot.save(context.store)
    return SimpleString("OK")


@command("BGSAVE", min_args=0, max_args=0, role=Role.ADMIN, complexity="O(N)", is_mutation=False, description="Asynchronously save the dataset to disk")
...
@command("BGREWRITEAOF", min_args=0, max_args=0, role=Role.ADMIN, complexity="O(N)", is_mutation=False, description="Asynchronously rewrite the append-only file")
def bgrewriteaof_cmd(args: List[Union[bytes, str]], context: CommandContext) -> SimpleString:
    if not context.aof:
        raise CommandError("ERR AOF persistence not configured")
    asyncio.create_task(context.aof.rewrite(context.store))
    return SimpleString("Background append only file rewriting started")


@command("LASTSAVE", min_args=0, max_args=0, role=Role.DEVELOPER, complexity="O(1)", is_mutation=False, description="Get the UNIX timestamp of the last successful save")
