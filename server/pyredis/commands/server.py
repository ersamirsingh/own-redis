...
@command("PING", min_args=0, max_args=1, role=Role.DEVELOPER, complexity="O(1)", is_mutation=False, description="Ping the server")
...
@command("ECHO", min_args=1, max_args=1, role=Role.DEVELOPER, complexity="O(1)", is_mutation=False, description="Echo the given string")
def echo_cmd(args: List[Union[bytes, str]], context: CommandContext) -> Any:
    return args[0]


@command("EXISTS", min_args=1, max_args=None, role=Role.DEVELOPER, complexity="O(N)", is_mutation=False, description="Determine if keys exist")
...
@command("TYPE", min_args=1, max_args=1, role=Role.DEVELOPER, complexity="O(1)", is_mutation=False, description="Determine the type stored at key")
...
@command("KEYS", min_args=1, max_args=1, role=Role.DEVELOPER, complexity="O(N)", is_mutation=False, description="Find all keys matching the given pattern")
...
@command("DBSIZE", min_args=0, max_args=0, role=Role.DEVELOPER, complexity="O(1)", is_mutation=False, description="Return the number of keys in the database")
def dbsize_cmd(args: List[Union[bytes, str]], context: CommandContext) -> int:
    return context.store.dbsize()


@command("FLUSHDB", min_args=0, max_args=0, role=Role.ADMIN, complexity="O(N)", is_mutation=True, description="Remove all keys from the current database")
